...
print ('\n\nreading kddcup.target dataset')
dfTarget = pd.read_csv('kddcup.target.txt')
print ('\n\nassigning clusters to original samples from kddcup data')
cluster = kmeans.predict(df_main_normalized)



# dfTarget=dfTarget.head(500)




print ('\n\nhere are the results of crosstab to show clusters vs target values')
print(pd.crosstab(cluster,dfTarget["target"]).to_string())

print ('\n\nthis shows the percentage of each target in each cluster')
df_main_normalized['cluster']=cluster
df_main_normalized['target']=dfTarget["target"]
g1 = df_main_normalized.groupby(['cluster','target'])
g2=df_main_normalized.groupby(['cluster'])
print (g1.size()*100/g2.size())
print ('\n\nthis shows the number of observations in each cluster')
print(g2.size())