...
public class TestEnv {
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        Env B2 = new Env(B1);
        B2.Put("w", FLOAT);                                                          // B1->B2
        B2.Put("y", BOOL );                                                          // B1->B2
        B2.Put("z", INT  );                                                          // B1->B2
        Assert(B2.Get("w") == FLOAT,"finding the float symbol 'w' in symbol table"); // B1->B2
        Assert(B2.Get("x") == INT  ,"finding the   int symbol 'x' in symbol table"); // B1->B2
        Assert(B2.Get("y") == BOOL ,"finding the  bool symbol 'y' in symbol table"); // B1->B2
        Assert(B2.Get("z") == INT  ,"finding the   int symbol 'z' in symbol table"); // B1->B2
